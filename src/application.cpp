...
void App::processKey(const sf::Event event)
{
	std::vector<sequence> seqs{
		sequence{{leader, "c", "L"}, [&] {window->close(); }},
	...
	default:
		...
		const auto seq = std::find_if(seqs.begin(), seqs.end(), [&](sequence s) {
...
void App::drawKeysWindow() {
	ImGui::Begin("Keys");
	if (keySeq != "")
	{
		ImGui::Text("Key: %s", keySeq.c_str());
