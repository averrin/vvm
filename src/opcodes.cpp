...
address Core::_INT(const std::byte code)
{
...
address Core::_JMP(const address addr) {
	return writeCode(JMP_a, addr);
}

address Core::_JMP(const int offset) {
	return writeCode(JMP_r, offset);
}

address Core::_JNE(const address addr) {
	return writeCode(JNE_a, addr);
}

address Core::_JNE(const int offset) {
	return writeCode(JNE_r, offset);
}

address Core::_INC(const address addr) {
	return writeCode(INC, addr);
}
address Core::_DEC(const address addr) {
	return writeCode(DEC, addr);
}

address Core::_PUSH(const address addr) {
	return writeCode(PUSH_m, addr);
}

address Core::_PUSH(const int value) {
	return writeCode(PUSH_c, value);
}

address Core::_POP(const address addr) {
