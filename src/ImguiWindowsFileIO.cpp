...
string MiniPath::filePath() const { return path + name; }
...
string MiniPath::getName() const { return name; }

string MiniPath::getPath() const { return path; }
  ...
  if (directory != NULL) {
    ...
    while ((entry = readdir(directory)) != NULL) {
      if (entry->d_type == DT_REG) {
        ...
        if (filter.empty() || filtersplit.back().compare("*") == 0 ||
            splits.back().compare(filtersplit.back()) == 0)
          directories.push_back(string(entry->d_name));
      }
    }
...
bool fileIOWindow(string &file_path, std::vector<string> &recently_used_files,
                  const string &button_text,
                  std::vector<std::string> file_filter, bool ensure_file_exists,
                  ImVec2 size) {
  bool close_it = false;
  ...
  list<string> directories =
      MiniPath::listDirectories(current_mini_path.getPath());
  ...
  list<string> local_files =
      MiniPath::listFiles(current_mini_path.getPath(),
                          string(extension_cstrings[file_type_selected]));
  vector<const char *> file_list = toCStringVec(local_files);

  if (directory_browsing)
    size.y += std::min(size_t(8), std::max(dir_list.size(), file_list.size())) *
              GetFontSize();
  ...
  if (!recently_used_files.empty()) {
    ...
    if (ImGui::BeginPopup("RecentFiles")) {
      if (ListBox("", &recent_selected, recent.data(), recent.size())) {
        current_mini_path.fromString(recently_used_files[recent_selected],
                                     sys_delim[0]);
  ...
  if (CollapsingHeader("Browse Directories")) {
    ...
    if (ListBox(" ", &directory_selected, dir_list.data(), dir_list.size())) {
      ...
      if (string(dir_list[directory_selected]).find("..") !=
          std::string::npos) {
        vector<string> split_directories = current_mini_path.getPathTokens();
        ...
        current_folder =
            current_folder + sys_delim + dir_list[directory_selected];
    }
  ...
  if (ImGui::BeginPopup("FileType")) {
    if (ListBox("", &file_type_selected, extension_cstrings.data(),
                extension_cstrings.size())) {
      CloseCurrentPopup();
    }
    ImGui::EndPopup();
  }
  ...
  if (Button(button_text.c_str())) {
    vector<string> ext_filter_v =
        stringSplit(extension_cstrings[file_type_selected], '.');
