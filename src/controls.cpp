...
void App::processKey(const sf::Event event) {
  ...
  default:
    ...
    statusMsg = "";
    const auto name = getKeyName(event.key.code, event.key.alt,
                                 event.key.control, event.key.shift);
    pressed_seq.push_back(name);

    std::ostringstream oss;
    std::copy(pressed_seq.begin(), pressed_seq.end(),
              std::ostream_iterator<std::string>(oss, " -> "));

    keySeq = oss.str();

    const auto seq =
        std::find_if(actions.begin(), actions.end(), [&](sequence s) {
          if (s.keys.size() < pressed_seq.size()) {
            return false;
          }
          auto res = true;
          auto n = 0;
...
std::string App::getKeyName(const sf::Keyboard::Key key, const bool alt,
  ...
  if (control) {
    mods = fmt::format("{}control+", mods);
  }
  if (shift) {
    mods = fmt::format("{}shift+", mods);
  }
  switch (key) {
  default:
